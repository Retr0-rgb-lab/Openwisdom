---
name: the-fool
description: "Steelman first, then challenge: Socratic assumptions, dialectic, pre-mortem, red team, falsification."
layer: scenario
scope: community
disciplines: ["philosophy"]
language: "en"
tags: ["steelman", "dialectic", "falsification", "critical-thinking", "curated-external", "openwisdom"]
version: "0.1.0"
license: "MIT"
metadata:
  openwisdom: true
  provenance: "curated-external"
  upstream: "https://github.com/jeffallan/claude-skills/tree/main/skills/the-fool"
---
# The Fool (Critical Reasoning)

## Summary

Steelman first, then challenge: Socratic assumptions, dialectic, pre-mortem, red team, falsification.

## Upstream

https://github.com/jeffallan/claude-skills/tree/main/skills/the-fool

This skill was curated into Openwisdom for discovery and local install.
Prefer the upstream repository for the latest author intent and license terms.

## Provenance

- Author: Jeff Allan
- Attribution: jeffallan/claude-skills
- License: MIT
- Openwisdom provenance: curated-external (installable community pack)

## Agent instructions

You are applying this Openwisdom skill in the current coding-agent session.
Follow the summary and steps above. Do not invent hosted Openwisdom chat.
Analysis runs here in this agent, not on openwisdom.vercel.app.
