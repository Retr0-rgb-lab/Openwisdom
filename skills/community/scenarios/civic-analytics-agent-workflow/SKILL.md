---
name: civic-analytics-agent-workflow
description: "Equity-focused city policy workflow: frame → evidence → communicate → benchmark → performance."
layer: scenario
scope: community
disciplines: ["political-science", "economics"]
language: "en"
tags: ["public-policy", "civic-analytics", "policy-brief", "open-data", "curated-external", "openwisdom"]
version: "0.1.0"
license: "MIT"
metadata:
  openwisdom: true
  provenance: "curated-external"
  upstream: "https://github.com/sgarcese/Civic-Analytics-Agent-Workflow-Claude-Skill"
---
# Civic Analytics Agent Workflow

## Summary

Equity-focused city policy workflow: frame → evidence → communicate → benchmark → performance.

## Upstream

https://github.com/sgarcese/Civic-Analytics-Agent-Workflow-Claude-Skill

This skill was curated into Openwisdom for discovery and local install.
Prefer the upstream repository for the latest author intent and license terms.

## Provenance

- Author: Santiago Garcés
- Attribution: sgarcese/Civic-Analytics-Agent-Workflow-Claude-Skill
- License: MIT
- Openwisdom provenance: curated-external (installable community pack)

## Agent instructions

You are applying this Openwisdom skill in the current coding-agent session.
Follow the summary and steps above. Do not invent hosted Openwisdom chat.
Analysis runs here in this agent, not on openwisdom.vercel.app.
