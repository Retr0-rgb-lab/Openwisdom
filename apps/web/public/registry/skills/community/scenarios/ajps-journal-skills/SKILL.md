---
name: ajps-journal-skills
description: "AJPS-oriented pack: topic, design, analysis, replication package, submission."
layer: scenario
scope: community
disciplines: ["political-science"]
language: "en"
tags: ["journal", "replication", "APSA", "methodology", "curated-external", "openwisdom"]
version: "0.1.0"
license: "MIT"
metadata:
  openwisdom: true
  provenance: "curated-external"
  upstream: "https://github.com/brycewang-stanford/Awesome-Journal-Skills/tree/main/American-Journal-of-Political-Science-Skills"
---
# American Journal of Political Science Skills

## Summary

AJPS-oriented pack: topic, design, analysis, replication package, submission.

## Upstream

https://github.com/brycewang-stanford/Awesome-Journal-Skills/tree/main/American-Journal-of-Political-Science-Skills

This skill was curated into Openwisdom for discovery and local install.
Prefer the upstream repository for the latest author intent and license terms.

## Provenance

- Author: brycewang-stanford
- Attribution: brycewang-stanford/Awesome-Journal-Skills
- License: MIT
- Openwisdom provenance: curated-external (installable community pack)

## Agent instructions

You are applying this Openwisdom skill in the current coding-agent session.
Follow the summary and steps above. Do not invent hosted Openwisdom chat.
Analysis runs here in this agent, not on openwisdom.vercel.app.
