---
name: adversaria-steelman
description: "Steelman → crucible → verdict with a persistent argument graph across dissent traditions."
layer: scenario
scope: community
disciplines: ["philosophy", "psychology"]
language: "en"
tags: ["argument-mapping", "steelman", "dialectic", "debate", "curated-external", "openwisdom"]
version: "0.1.0"
license: "MIT"
metadata:
  openwisdom: true
  provenance: "curated-external"
  upstream: "https://github.com/damionrashford/Adversaria/tree/main/skills/steelman"
---
# Adversaria Argument Graph

## Summary

Steelman → crucible → verdict with a persistent argument graph across dissent traditions.

## Upstream

https://github.com/damionrashford/Adversaria/tree/main/skills/steelman

This skill was curated into Openwisdom for discovery and local install.
Prefer the upstream repository for the latest author intent and license terms.

## Provenance

- Author: Damion Rashford
- Attribution: damionrashford/Adversaria
- License: MIT
- Openwisdom provenance: curated-external (installable community pack)

## Agent instructions

You are applying this Openwisdom skill in the current coding-agent session.
Follow the summary and steps above. Do not invent hosted Openwisdom chat.
Analysis runs here in this agent, not on openwisdom.vercel.app.
